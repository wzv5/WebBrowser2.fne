...
CWnd* GetWndPtr2 (PMDATA_INF pInf)
{
	return (CWnd*)NotifySys (NRS_GET_UNIT_PTR, pInf->m_unit.m_dwFormID,
			pInf->m_unit.m_dwUnitID);
}
...
void SerializeFont (LOGFONT* str, CArchive& ar)
{
	INT nSize;
	if (ar.IsStoring() == TRUE)
	{
		nSize = sizeof (LOGFONT);
		ar << nSize;
		if (nSize > 0)
			ar.Write ((LPBYTE)str, nSize);
	}
	else
	{
		ar >> nSize;
